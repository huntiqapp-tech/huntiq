...
## DONE / PRESENT
- **Cloudflare Workers Think stub (not deployed):** `workers/huntiq-agent/` is a self-contained Workers package for future gethuntiq.com use. `HuntiqAgent` extends `Think` from `@cloudflare/think` and owns each chat turn. Public `/`, `/health`, and `/enp` are handled in `src/http.ts` before Think routing. Customer `/enp` and tool calls fail closed unless `compKind` is sold and `soldCompsConfirmed` is true; asking/active listings, MSRP, rebates, and store credit cannot set ENP. Math is contract-tested against `lib/enp-calculator.js`; HTTP integration tests cover malformed JSON, missing gates, asking prices, and sold success. `AGENT.md` snapshots the customer-facing source so truncated review packets still see it. Workers tracing is enabled at `head_sampling_rate = 1` with metadata-only content (`storeMessages` / `storeTools` are not set true). The public GitHub Pages PWA and `calculator.html` are unchanged. Do not `wrangler deploy` until a human attaches an account and, later, a custom domain.
