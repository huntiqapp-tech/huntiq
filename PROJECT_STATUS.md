...
- [x] Browser now loads `resale-history`, `evidence-gate`, `opportunity-evaluator`, and the PWA bridge/runtime; the offline service-worker cache was bumped to `huntiq-public-v41` and includes those modules.
...
- [x] PWA bridge tests (`tests/pwa-opportunity.test.js`) are wired into the main suite; package version is 0.9.22.
...
## CURRENT REALITY
The public preview is not yet a fully live deal engine. Demonstration opportunity data is intentionally used and should remain clearly labeled until replaced by verified integrations. The browser path now uses the strict evaluator for displayed market/profit/ROI and alert eligibility, but the demo preview still derives completed-sale fixtures from demo 30/60/90 summaries until a rights-cleared live sold-data provider is connected. Private credentials/backend services must not be committed to this public repository.
