...
assert(/if\(demo\)\s*\{\s*const step=7\*24\*36e5;/.test(app), 'synthetic weekly cadence may only be used for demo history');
assert(app.includes('HuntIQCustomerAppBoundary.filterHistoryObservations('), 'live historical rows must pass strict retailer/product/channel/location filtering');
assert(app.includes('{before:observedAt}'), 'live historical rows must predate the current observation');
