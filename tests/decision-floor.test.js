...
const strong=[{evidenceQuality:.99,verified:true,direct:true,sourceType:'official-api',retentionPolicy:'persistent',redistributionAllowed:true,ageHours:1,identityMatched:true}];
{
  ...
  const expected=Math.min(...[e.sourceAdjustedRoi,e.sourceAdjustedDownsideRoi,e.sourceAdjustedConfidenceAdjustedRoi,e.decayAdjustedRoi,e.marketplaceLateSaleRoi].filter(Number.isFinite));
  ...
  assert.equal(e.decisionFloorBasis,'minimum-of-risk-downside-confidence-source-decay-marketplace-late-sale-adjusted');
