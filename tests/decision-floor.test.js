...
{
  const out=evaluateOpportunity({opportunity:{price:45,taxRate:.06,shipping:6,returnRate:.03,returnShipping:6,returnHandlingCost:2},comparables:[sold(130,2),sold(126,5),sold(128,8),sold(122,12),sold(120,17),sold(118,24),sold(116,35),sold(114,50)],channels:[{name:'eBay',feeRate:.135,fixedFee:.4,holdingDays:18}],history:{sampleCount:16,spanDays:60,confidence:94,historyCoverageScore:96,madPct:2,regimeStabilityScore:94},anomaly:{confidence:93,phase:'new-drop',madPct:2,regimeStabilityScore:94},deal:{fresh:true,verified:true},sourceEvidence:{retailer:strong,resale:strong},asOf});
  const e=out.economics;
  const expected=Math.min(...[e.sourceAdjustedRoi,e.sourceAdjustedDownsideRoi,e.sourceAdjustedConfidenceAdjustedRoi,e.decayAdjustedRoi,e.marketplaceLateSaleRoi,e.marketplaceUncertaintyRoi,e.marketSpreadAdjustedRoi].filter(Number.isFinite));
  ...
  assert(Number.isFinite(e.marketSpreadAdjustedRoi),'market spread ROI should be present in evaluator economics');
  assert.equal(e.decisionFloorBasis,'minimum-of-risk-downside-confidence-source-decay-marketplace-late-sale-uncertainty-market-spread-adjusted');
