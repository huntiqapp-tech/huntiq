...
const resale=E.resaleSummary({d30:500,d60:480,d90:460,soldCount:30,soldWindowDays:90,activeListingCount:12,currentAsks:[520,540,500]});
...
assert(resale.sellThroughRate>60);
assert(resale.liquidityScore>50);
assert.strictEqual(resale.activeListingCount,12);

const liquid=E.liquiditySummary({soldCount:45,soldWindowDays:30,activeListingCount:8});
const slow=E.liquiditySummary({soldCount:3,soldWindowDays:90,activeListingCount:40});
assert(liquid.liquidityScore>slow.liquidityScore);
assert(liquid.estimatedDaysToSell<slow.estimatedDaysToSell);
assert(liquid.sellThroughRate>slow.sellThroughRate);
...
const fastCapital=E.capitalEfficiency(econ,4);
const slowCapital=E.capitalEfficiency(econ,60);
assert(fastCapital.profitPerDay>slowCapital.profitPerDay);
assert(fastCapital.roi30Equivalent>slowCapital.roi30Equivalent);
assert(fastCapital.score>slowCapital.score);
...
const evaluated=E.evaluateOpportunity({price:129,referencePrice:599,priceHistory:[599,599,579,599,599,599,589,599,599,599,599,599,599,599],comps:{d30:489,d60:475,d90:469,soldCount:38,soldWindowDays:90,activeListingCount:14},feeRate:0.135,shipping:24,taxRate:0.06,holdingCostPerDay:.1,confirmationScore:90,evidenceQuality:.95});
...
assert(evaluated.resale.liquidityScore>0);
assert(evaluated.capitalEfficiency.score>0);
...
console.log('HUNTIQ engine tests passed', {flipScore:evaluated.flipScore, roi:evaluated.economics.roi, riskRoi:evaluated.riskAdjustedEconomics.roi, liquidity:evaluated.resale.liquidityScore, capital:evaluated.capitalEfficiency.score, downside:evaluated.downsideEconomics.profit, anomaly:evaluated.anomaly.confidence});