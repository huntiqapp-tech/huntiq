...
const unlabeled=classifyOpportunityData({observedAt:asOf},{asOf});
assert.equal(unlabeled.kind,'validation');assert.equal(unlabeled.customerVisible,false);assert.equal(unlabeled.reason,'unlabeled-origin');
const partial=classifyOpportunityData({dataOrigin:'live',validationState:'validated',observedAt:asOf,evidenceAuthority:{historyAuthoritative:true}},{asOf});
assert.equal(partial.kind,'validation');assert.equal(partial.customerVisible,false);assert.match(partial.reason,/customer-authority-incomplete/);
