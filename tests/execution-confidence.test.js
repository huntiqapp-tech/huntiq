...
assert.equal(scoreOpportunityConfidence({...base,executionConfidence:strong}).alertEligible,true);
const gated=scoreOpportunityConfidence({...base,executionConfidence:single});assert.equal(gated.alertEligible,false);assert(gated.blockers.includes('extreme anomaly lacks repeat confirmation'));
