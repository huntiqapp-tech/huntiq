...
const SECRET_QUERY_PARAM = /(authorization|api[-_]?key|token|secret|password)/i;

function stripSecretQueryParams(url) {
  for (const key of [...url.searchParams.keys()]) {
    if (SECRET_QUERY_PARAM.test(key)) url.searchParams.delete(key);
  }
  return url;
}
...
async function fetchHtml(request, {
  ...
  maxResponseBytes = DEFAULT_POLICY.maxResponseBytes,
  allowedHosts = []
} = {}) {
  ...
  try {
    ...
    if (response.url) validateTargetUrl(response.url, { allowedHosts });
    ...
    if (isAbortError(error)) throw new Error(`retailer request timed out after ${timeoutMs}ms`);
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
async function scrapeRetailerProduct({
  ...
  const html = await fetchHtml(request, { fetchImpl, timeoutMs, maxResponseBytes, allowedHosts });
