const { attachObservationAliases, normalizeLiveObservation, redactSecrets } = require('./live-ingestion');
...
const DEFAULT_POLICY = Object.freeze({
  ...
  timeoutMs: 12_000,
  maxAgeMinutes: 15,
  rightsClass: 'public-page-internal-validation',
  retentionPolicy: 'ephemeral'
});

const SECRET_NAME = /(authorization|cookie|api[-_]?key|token|secret|password)/i;
...
function validateTargetUrl(value, { allowedHosts = [] } = {}) {
  ...
  if (!allow.length) throw new Error('an explicit retailer host allowlist is required');
  if (host === 'localhost' || isIP(host) || url.port) throw new Error('private, IP, localhost, and custom-port targets are not allowed');
  if (!allow.some((entry) => host === entry || host.endsWith(`.${entry}`))) {
    throw new Error(`retailer host is not allowlisted: ${host}`);
  }
  for (const key of url.searchParams.keys()) {
    if (SECRET_NAME.test(key)) throw new Error('credential-bearing query parameters are not allowed');
  }
...
function buildScrapeRequest({ url, allowedHosts = [], headers = {} } = {}) {
  ...
  if (Object.keys(headers).some((name) => SECRET_NAME.test(name))) throw new Error('credential-bearing scraper headers are not allowed');
  return {
    url: targetUrl,
    method: 'GET',
...
async function scrapeRetailerProduct({
  ...
  retentionPolicy = DEFAULT_POLICY.retentionPolicy,
  provider = 'huntiq-scraper',
  maxAgeMinutes = DEFAULT_POLICY.maxAgeMinutes,
  fetchImpl,
  headers,
  timeoutMs,
  maxResponseBytes
  ...
  const retrievedAt = new Date(asOf || Date.now());
  const ageMinutes = (retrievedAt.getTime() - Date.parse(observation.observedAt)) / 60000;
  if (!Number.isFinite(ageMinutes)) throw new Error('valid scraper observation time is required');
  if (ageMinutes < -5) throw new Error('future scraper observation is not allowed');
  if (ageMinutes > maxAgeMinutes) throw new Error('stale scraper observation is not allowed');
  return attachObservationAliases({
    ...
    source: {
      ...observation.source,
      providerRecordId: request.url,
      retrievedAt: retrievedAt.toISOString(),
      retentionPolicy,
      redistributionAllowed: false,
      rawProvenance: {
        evidenceUrl: request.url,
        extractor: product.sourceType,
        retrievedAt: retrievedAt.toISOString()
      }
    }
  });
