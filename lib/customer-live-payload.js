...
function toCustomerOpportunity(assessment = {}, { asOf = new Date().toISOString(), enableAlerts = false, validation = {} } = {}) {
  ...
  const historyPromoted = historyEvidence.historyPromoted === true || assessment.historyPromotion?.historyEligible === true;
  const promotedCount = Number.isFinite(Number(historyEvidence.promotedCount))
    ? Number(historyEvidence.promotedCount)
    : (historyPromoted ? priceHistory.length : 0);
  ...
  const readiness = assessLiveOpportunityReadiness({
    ...
    historyEvidence: {
      sampleCount: Number.isFinite(Number(historyEvidence.sampleCount)) ? Number(historyEvidence.sampleCount) : priceHistory.length,
      promotedCount,
...
function buildCustomerLivePayload(batch, validation, options = {}) {
  ...
  for (const assessment of batch.assessments) {
    try {
      opportunities.push(toCustomerOpportunity(assessment, { asOf, enableAlerts: options.enableAlerts === true, validation }));
    } catch (error) {
      rejected.push({
