...
function assertValidationCoversObservation(observation = {}, validation = {}, asOf = null) {
  ...
  const validatedTime = Date.parse(validatedAt);
...
function sanitizeCompletedSales(assessment = {}, observation = {}, { asOf = null } = {}) {
  ...
  return rows.map(sale => {
    ...
    if (sale?.verified !== true) return null;
...
function toCustomerOpportunity(assessment = {}, { asOf = new Date().toISOString(), enableAlerts = false, validation = {}, expectedProvider = null } = {}) {
  ...
  const timing = assertValidationCoversObservation(observation, validation, asOf);
  ...
  const resaleAggregatesAuthorized = compIdentityMatches && completedSales.length >= 3;
  const comps = {
  ...
  return {
    ...
    referencePrice: readiness.historyReady && Number.isFinite(rawReferencePrice) ? rawReferencePrice : null,
    priceHistory,
...
module.exports = {
  ...
  assertValidationCoversObservation,
  publicSource,
