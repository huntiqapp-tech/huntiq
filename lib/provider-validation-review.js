...
function sameIdentity(expected = {}, actual = {}) {
  ...
  for (const key of keys) {
    ...
    if (!left || !right) { ambiguous.push(key); continue; }
    if (left.toLowerCase() !== right.toLowerCase()) mismatches.push(key);
  }
  return { exact: mismatches.length === 0 && ambiguous.length === 0, mismatches, ambiguous };
}

function observationHash(observation = {}) {
  const stable = {
    retailer: clean(observation.retailer), productId: clean(observation.productId), sku: clean(observation.sku),
    upc: clean(observation.upc), modelNumber: clean(observation.modelNumber), storeId: clean(observation.storeId),
    zip: clean(observation.zip), channel: clean(observation.channel), price: finitePrice(observation.price),
...
function reviewProviderValidationRun(run = {}, evidence = {}, options = {}) {
  ...
  const rightsClass = clean(evidence.rightsClass);
  const retentionPolicy = clean(evidence.retentionPolicy);
  const provider = normalizeProvider(run.provider);
  const runRetailer = clean(run.retailer);
  ...
  if (!displayAllowed) cautions.push('customer-display-rights-unverified');
  if (!retentionAllowed) blockers.push('retention-rights-unverified');
  if (!redistributionAllowed) cautions.push('redistribution-rights-unverified');
  if (!rightsClass || rightsClass.toLowerCase() === 'unknown') blockers.push('rights-class-required');
  if (!retentionPolicy || ['unknown', 'ephemeral'].includes(retentionPolicy.toLowerCase())) blockers.push('persistent-retention-policy-required');

  const providerObservation = evidence.providerObservation || {};
  const sourceObservation = evidence.sourceObservation || {};
  const observationProvider = normalizeProvider(providerObservation.source?.provider);
  if (!['retailerapi', 'bright-data'].includes(provider)) blockers.push('unsupported-provider');
  if (!observationProvider || observationProvider !== provider) blockers.push('provider-provenance-mismatch');
  if (!runRetailer || !clean(providerObservation.retailer) || runRetailer.toLowerCase() !== clean(providerObservation.retailer).toLowerCase()) {
    blockers.push('retailer-provenance-mismatch');
  }
  if (!clean(run.snapshotId || run.snapshot_id) && !clean(providerObservation.source?.providerRecordId)) {
    blockers.push('provider-record-provenance-required');
  }
  ...
  if (providerPrice == null || sourcePrice == null) blockers.push('manual-price-comparison-required');
  else {
    const toleranceCents = Number.isFinite(Number(options.priceToleranceCents)) ? Math.max(0, Number(options.priceToleranceCents)) : 1;
    if (Math.round(Math.abs(providerPrice - sourcePrice) * 100) > toleranceCents) blockers.push('manual-price-mismatch');
  }
  ...
  return {
    provider, retailer: runRetailer, snapshotId: clean(run.snapshotId || run.snapshot_id),
    validationState: validated ? 'validated' : 'shadow-review-required', manualSourceCheckPassed: manualPassed,
    customerDisplayAllowed: validated && displayAllowed && redistributionAllowed, retentionAllowed: validated && retentionAllowed,
    redistributionAllowed: validated && redistributionAllowed, historyPromotionAllowed: validated && retentionAllowed,
    rightsClass, retentionPolicy, alertsEnabled: false, identity, checkedAt, blockers, cautions
  };
}
...
function promoteValidatedObservation(observation = {}, review = {}) {
  ...
  return {
    ...observation, price, observedAt, validationState: 'validated-history', historyEligible: true, alertEligible: false,
    observationHash: observationHash({ ...observation, price, observedAt }),
    source: {
      ...(observation.source || {}), validationState: 'validated', manualSourceCheckPassed: true,
      customerDisplayAllowed: review.customerDisplayAllowed === true, retentionAllowed: review.retentionAllowed === true,
      redistributionAllowed: review.redistributionAllowed === true, rightsClass: review.rightsClass,
      retentionPolicy: review.retentionPolicy, alertsEnabled: false
    }
  };
}

module.exports = { normalizeProvider, sameIdentity, observationHash, reviewProviderValidationRun, promoteValidatedObservation };
