...
function reviewProviderValidationRun(run = {}, evidence = {}, options = {}) {
  ...
  return {
    provider: clean(run.provider), retailer: clean(run.retailer), snapshotId: clean(run.snapshotId || run.snapshot_id),
    validationState: validated ? 'validated' : 'shadow-review-required', manualSourceCheckPassed: manualPassed,
    customerDisplayAllowed: validated && displayAllowed, retentionAllowed: validated && retentionAllowed,
...
module.exports = { sameIdentity, observationHash, reviewProviderValidationRun, promoteValidatedObservation };
