...
function selectVisibleOpportunities(opportunities=[], {asOf=new Date().toISOString(),alertsEnabled=false,fromDemoFallback=false}={}){
  ...
  for(const raw of opportunities){
    ...
    if(!row)continue;
    const dataState=classifyRow(row,{asOf});
...
return{
  HOUR,detectAppMode,readSuppliedFeed,coerceIncoming,locationLabel,freshnessLabel,
  ...
  authorizedCompsForEconomics,customerEconomicsAuthorized
};
});
