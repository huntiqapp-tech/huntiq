...
function evaluateMultiBuyPromotion(input={}){
  ...
  const eligible=reasons.length===0;const itemDiscounts={};let totalDiscount=0;
  ...
  const limitUncertain=eligible&&!policy.known&&policy.uncertain>0;const status=!eligible?(reasons.some(r=>r.includes('unconfirmed'))?'unknown':'ineligible'):(limitUncertain?'unknown':'eligible');
  if(eligible&&totalDiscount>0)warnings.push('Multi-buy savings require purchasing the full qualifying quantity.');
  if(eligible&&policy.known&&availableRedemptions>appliedRedemptions)warnings.push('Promotion redemption limit capped the available multi-buy savings.');
  if(limitUncertain)warnings.push('Retailer says a redemption limit may apply; HUNTIQ credits only the guaranteed redemption count and treats additional savings as uncertain.');
