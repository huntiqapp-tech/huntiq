...
function scoreOpportunityConfidence(deal={}){
  const anomaly=deal.anomaly||{};const history=deal.historyAssessment||deal.history||{};const resale=deal.resale||{};const economics=deal.economics||{};const downside=deal.downsideEconomics||deal.downside||{};const alert=deal.alert||{};const agreement=deal.evidenceAgreement||null;
  ...
  if(agreement){
    const agreementScore=clamp(agreement.score);
    if((agreement.blockers||[]).length||agreementScore<60){
      blockers.push(...((agreement.blockers||[]).length?agreement.blockers:['cross-source evidence disagreement']));
      score=Math.min(score,59);
    }else if(agreementScore<80){
      cautions.push(...((agreement.cautions||[]).length?agreement.cautions:['mixed cross-source evidence']));
  ...
  const alertEligible=alert.alert===true&&level==='HIGH'&&!blockers.length&&origin!=='demo'&&(!agreement||agreement.alertEligible!==false);
  return{score:Math.round(score),level,components,weakestComponent:Object.entries(components).sort((a,b)=>a[1]-b[1])[0][0],blockers:[...new Set(blockers)],cautions:[...new Set(cautions)],alertEligible,method:'weighted-evidence-with-weak-link-and-agreement-gates'};
}
