...
function scoreOpportunityConfidence(deal={}){
  const anomaly=deal.anomaly||{};const history=deal.historyAssessment||deal.history||{};const resale=deal.resale||{};const economics=deal.economics||{};const downside=deal.downsideEconomics||deal.downside||{};const alert=deal.alert||{};const agreement=deal.evidenceAgreement||null;const range=deal.opportunityRange||null;
  ...
  const alertEligible=alert.alert===true&&level==='HIGH'&&!blockers.length&&origin!=='demo'&&(!agreement||agreement.alertEligible!==false)&&(!range||range.alertEligible!==false);
  return{score:Math.round(score),level,components,weakestComponent:Object.entries(components).sort((a,b)=>a[1]-b[1])[0][0],blockers:[...new Set(blockers)],cautions:[...new Set(cautions)],alertEligible,method:'weighted-evidence-with-weak-link-agreement-and-profit-range-gates'};
}
