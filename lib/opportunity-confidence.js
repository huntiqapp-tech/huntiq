...
function scoreOpportunityConfidence(deal={}){
  ...
  const alertEligible=alert.alert===true&&level==='HIGH'&&!blockers.length&&origin!=='demo'&&(!agreement||agreement.alertEligible!==false)&&(!range||range.alertEligible!==false)&&(!execution||execution.alertEligible!==false);
  return{score:Math.round(score),level,components,weakestComponent:Object.entries(components).sort((a,b)=>a[1]-b[1])[0][0],blockers:[...new Set(blockers)],cautions:[...new Set(cautions)],alertEligible,method:'weighted-evidence-with-agreement-range-and-execution-gates'};
}
