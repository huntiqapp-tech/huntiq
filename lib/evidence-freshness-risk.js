...
function assessEvidenceFreshnessRisk(input={}){
  ...
  const integrity=clamp(input.observationIntegrityScore??100);
  const anomaly=clamp(input.anomalyConfidence??50);
  const resale=clamp(input.resaleConfidence??50);
  const projectedResale=Math.max(0,Number(input.projectedResale)||0);
  ...
  const integrityFactor=.55+.45*(integrity/100);
  const overallFreshness=clamp((acquisitionFreshness*.35+priceHistoryFreshness*.30+resaleFreshness*.35)*integrityFactor);
  const adjustedAnomalyConfidence=clamp(anomaly*(.55+.45*(priceHistoryFreshness/100))*(.75+.25*(acquisitionFreshness/100))*integrityFactor);
  const adjustedResaleConfidence=clamp(resale*(.55+.45*(resaleFreshness/100))*integrityFactor);
  const resaleHaircut=clamp((100-resaleFreshness)*.0025+(100-integrity)*.001,0,.30);
  ...
  const blocked=integrity<45||acquisitionFreshness<40||priceHistoryFreshness<30||resaleFreshness<35||freshnessAdjustedProfit<=0||freshnessAdjustedRoi<=0;
  ...
  const reasons=[];
  if(integrity<70)reasons.push('weak-observation-integrity');
