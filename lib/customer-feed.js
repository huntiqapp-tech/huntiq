...
function rankOpportunity({health={},presentation={},deal={}}={}){
...
function buildFeed(items=[]){
  ...
  return{
    topPicks:ranked.filter(x=>(x.health||{}).state==='BUY-READY'&&x.feedPriority>=72),
    watch:ranked.filter(x=>(x.health||{}).state==='WATCH'),
    blocked:ranked.filter(x=>(x.health||{}).state==='BLOCKED'),
    all:ranked
  };
}
