(function(root,factory){const api=factory(typeof module==='object'&&module.exports?require('./basket-promotions'):root.HuntIQBasketPromotions,typeof module==='object'&&module.exports?require('./multibuy-promotions'):root.HuntIQMultiBuyPromotions);if(typeof module==='object'&&module.exports)module.exports=api;root.HuntIQAcquisitionCost=api;})(typeof globalThis!=='undefined'?globalThis:this,function(BasketPromotions,MultiBuyPromotions){
'use strict';
...
function evaluateMultiBuyPromotion(acquisition={}){
  if(!acquisition.multiBuyPromotion)return null;
  if(!MultiBuyPromotions||typeof MultiBuyPromotions.evaluateMultiBuyPromotion!=='function')return{status:'unknown',eligible:false,currentItemDiscount:0,reasons:['multibuy-engine-unavailable'],warnings:[]};
  return MultiBuyPromotions.evaluateMultiBuyPromotion(acquisition.multiBuyPromotion);
}
function evaluateAcquisition(opportunity={}){
  ...
  const multiBuyPromotion=evaluateMultiBuyPromotion(acquisition);
  const basketEligible=!basketPromotion||basketPromotion.eligible;
  const multiBuyEligible=!multiBuyPromotion||multiBuyPromotion.eligible;
  const combinedPromotionEligible=promotion.eligible&&basketEligible&&multiBuyEligible;
  const combinedPromotionStatus=!promotion.eligible?promotion.status:(basketPromotion&&!basketPromotion.eligible?basketPromotion.status:(multiBuyPromotion&&!multiBuyPromotion.eligible?multiBuyPromotion.status:'eligible'));
  const combinedPromotionReasons=[...promotion.reasons,...(basketPromotion?basketPromotion.reasons:[]),...(multiBuyPromotion?multiBuyPromotion.reasons:[])];
  ...
  const multiBuyDiscount=multiBuyPromotion&&multiBuyPromotion.eligible?Math.max(0,Number(multiBuyPromotion.currentItemDiscount)||0):0;
  ...
  const checkoutPrice=Math.max(0,stickerPrice-instantDiscount-checkoutCredit-(promotion.eligible?basketInstantDiscount+basketCheckoutCredit:0)-(promotion.eligible?multiBuyDiscount:0));
  ...
  if(multiBuyPromotion&&!multiBuyPromotion.eligible)warnings.push('Multi-buy savings excluded until the full qualifying quantity is confirmed.');
  if(multiBuyPromotion&&multiBuyPromotion.eligible&&multiBuyDiscount>0)warnings.push('Multi-buy savings apply only when the full qualifying quantity is purchased.');
  return{stickerPrice:money(stickerPrice),requestedInstantDiscount:money(requestedInstantDiscount),requestedCheckoutCredit:money(requestedCheckoutCredit),instantDiscount:money(instantDiscount+(promotion.eligible?basketInstantDiscount:0)+(promotion.eligible?multiBuyDiscount:0)),checkoutCredit:money(checkoutCredit+(promotion.eligible?basketCheckoutCredit:0)),checkoutPrice:money(checkoutPrice),taxRate:+taxRate.toFixed(4),taxablePrice:money(taxablePrice),purchaseTax:money(purchaseTax),cashOutlay:money(cashOutlay),requestedFutureCredit:money(futureCreditRequested),futureCredit:money(futureCredit),futureCreditType,realizationRate:+realizationRate.toFixed(3),daysToCredit:+daysToCredit.toFixed(1),annualDiscountRate:+annualDiscountRate.toFixed(4),timeValueFactor:+timeValueFactor.toFixed(4),expectedFutureCredit:money(expectedFutureCredit),economicAcquisitionCost:money(economicAcquisitionCost),hasDeferredValue:futureCredit>0,promotion,promotionEligible:combinedPromotionEligible,promotionStatus:combinedPromotionStatus,promotionReasons:combinedPromotionReasons,promotionWarnings:[...promotion.warnings,...(basketPromotion?basketPromotion.warnings:[]),...(multiBuyPromotion?multiBuyPromotion.warnings:[])],basketPromotion,basketPromotionEligible:basketPromotion?basketPromotion.eligible:true,basketPromotionStatus:basketPromotion?basketPromotion.status:'none',basketPromotionReasons:basketPromotion?basketPromotion.reasons:[],basketAllocatedReward:money(basketAllocation),basketRewardType:basketPromotion?basketRewardType:null,multiBuyPromotion,multiBuyPromotionEligible:multiBuyPromotion?multiBuyPromotion.eligible:true,multiBuyPromotionStatus:multiBuyPromotion?multiBuyPromotion.status:'none',multiBuyPromotionReasons:multiBuyPromotion?multiBuyPromotion.reasons:[],multiBuyDiscount:money(multiBuyDiscount),combinedPromotionEligible,warnings,warning:warnings[0]||null};
}
return{evaluatePromotion,evaluateBasketPromotion,evaluateMultiBuyPromotion,evaluateAcquisition};
});