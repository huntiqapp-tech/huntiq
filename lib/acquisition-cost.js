...
function evaluateAcquisition(opportunity={}){
  ...
  const futureCreditRequested=Math.max(0,Number(acquisition.futureCredit==null?acquisition.rebateCredit:acquisition.futureCredit)||0);
  const futureCredit=promotion.eligible?futureCreditRequested:0;
  ...
  if(!promotion.eligible&&(requestedInstantDiscount>0||requestedCheckoutCredit>0||futureCreditRequested>0))warnings.push('Promotion value excluded until eligibility is confirmed.');
  return{stickerPrice:money(stickerPrice),requestedInstantDiscount:money(requestedInstantDiscount),requestedCheckoutCredit:money(requestedCheckoutCredit),instantDiscount:money(instantDiscount),checkoutCredit:money(checkoutCredit),checkoutPrice:money(checkoutPrice),taxRate:+taxRate.toFixed(4),taxablePrice:money(taxablePrice),purchaseTax:money(purchaseTax),cashOutlay:money(cashOutlay),requestedFutureCredit:money(futureCreditRequested),futureCredit:money(futureCredit),futureCreditType,realizationRate:+realizationRate.toFixed(3),daysToCredit:+daysToCredit.toFixed(1),annualDiscountRate:+annualDiscountRate.toFixed(4),timeValueFactor:+timeValueFactor.toFixed(4),expectedFutureCredit:money(expectedFutureCredit),economicAcquisitionCost:money(economicAcquisitionCost),hasDeferredValue:futureCredit>0,promotion,promotionEligible:promotion.eligible,promotionStatus:promotion.status,promotionReasons:promotion.reasons,promotionWarnings:promotion.warnings,warnings,warning:warnings[0]||null};
}
return{evaluatePromotion,evaluateAcquisition};
});