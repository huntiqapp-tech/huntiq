...
function buildDealCoach(deal={}){
  ...
  const lifecycle=deal.clearanceLifecycle||deal.lifecycle||{};
  ...
  const lifecycleStage=String(lifecycle.stage||'').trim(),lifecycleProfit=num(lifecycle.lifecycleAdjustedProfit),lifecycleRoi=num(lifecycle.lifecycleAdjustedRoi),lifecycleResale=num(lifecycle.lifecycleAdjustedResale),lifecycleAnomalyConfidence=num(lifecycle.adjustedAnomalyConfidence);
  ...
  if(lifecycleStage==='fresh-markdown')reasons.push('Clearance lifecycle still looks like a fresh markdown rather than an established low-price regime.');
  if(lifecycleStage==='established-markdown')cautions.push('This is an established markdown, so urgency is lower than the headline discount suggests.');
  if(lifecycleStage==='normalized-low-price')cautions.push('This low price increasingly looks normalized; HUNTIQ is reducing fresh-anomaly confidence.');
  if(lifecycleStage==='terminal-clearance')cautions.push('Retailer-marked terminal clearance can increase resale competition as other buyers source the same item.');
  if(lifecycleAnomalyConfidence!=null&&anomalyConfidence!=null&&lifecycleAnomalyConfidence<anomalyConfidence)cautions.push(`Lifecycle-adjusted anomaly confidence is ${pct(lifecycleAnomalyConfidence)}.`);
  if(lifecycleResale!=null)cautions.push(`Lifecycle-adjusted resale estimate is about ${money(lifecycleResale)}.`);
  if(lifecycleProfit!=null&&lifecycleRoi!=null&&profit!=null&&lifecycleProfit<profit)cautions.push(`Lifecycle-adjusted economics fall to about ${money(lifecycleProfit)} profit at ${pct(lifecycleRoi)} ROI.`);
  if(Array.isArray(lifecycle.blockers))for(const blocker of lifecycle.blockers){const text=String(blocker||'').trim();if(text&&!cautions.includes(text))cautions.push(text);}
  ...
  if(lifecycle.alertEligible===false&&verdict==='BUY')verdict='WATCH';
