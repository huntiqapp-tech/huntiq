...
function assessMarketReality(deal={},resale={}){
  const acquisitionPrice=Math.max(0,Number(deal.price)||0);
  const soldMarketValue=Math.max(0,Number(resale.marketValue)||0);
  const referencePrice=Math.max(0,Number(deal.referencePrice??deal.msrp??deal.listPrice??deal.regularPrice??deal.compareAtPrice)||0);
  const spread=soldMarketValue>0?soldMarketValue-acquisitionPrice:0;
  const marketEdgePct=soldMarketValue>0?spread/soldMarketValue*100:null;
  const referenceDiscountPct=referencePrice>0?(referencePrice-acquisitionPrice)/referencePrice*100:null;
  let verdict='sold-market-unknown';
  if(soldMarketValue>0)verdict=spread>0?'below-sold-market':spread<0?'above-sold-market':'at-sold-market';
  return{
    acquisitionPrice:money(acquisitionPrice),soldMarketValue:money(soldMarketValue),marketSpread:money(spread),
    marketEdgePct:marketEdgePct==null?null:+marketEdgePct.toFixed(1),verdict,
    referencePrice:referencePrice>0?money(referencePrice):null,
    referenceDiscountPct:referenceDiscountPct==null?null:+referenceDiscountPct.toFixed(1),
    referencePriceAuthoritative:false,
    resaleBasis:resale.marketValueBasis||null,
    note:soldMarketValue>0?(spread>0?'Retail price is below verified sold-market value.':'Retail price has no positive spread versus verified sold-market value.'):'Verified sold-market value is unavailable; reference/MSRP cannot substitute.'
  };
}
function evaluateForPwa(deal={},legacy={},opts={}){
  ...
  const marketReality=assessMarketReality(deal,evaluated.resale);
  return{...evaluated,marketReality,sourceAdjustedEconomics:evaluated.economics,historyAssessment,comparables:rawComparables,asOf,demoComparables:isDemo&&!hasLiveCompletedSales};
}
return{buildDemoCompletedSales,resaleSourceEvidence,assessMarketReality,evaluateForPwa};
});