...
function evaluateForPwa(deal={},legacy={},opts={}){
  ...
  const hasLiveCompletedSales=Boolean(deal.completedSales&&deal.completedSales.length);const rawComparables=hasLiveCompletedSales?deal.completedSales:buildDemoCompletedSales(deal,asOf);
  ...
  const evaluated=OpportunityEvaluator.evaluateOpportunity({opportunity:{price:deal.price,taxRate:deal.taxRate||0,shipping:deal.shipping||0,miscCost:deal.miscCost||0,holdingCostPerDay:deal.holdingCostPerDay||0,returnRate:deal.returnRate||.05,returnShipping:deal.returnShipping==null?(deal.shipping||0):deal.returnShipping,returnHandlingCost:deal.returnHandlingCost||2,nonRefundableFeeRate:deal.nonRefundableFeeRate||.03},comparables:rawComparables,channels,history,anomaly:{confidence:strictAnomalyConfidence,phase:anomaly.phase||'unknown'},deal:{fresh:historyAssessment?historyAssessment.freshnessScore>=50:true,verified:true},sourceEvidence,asOf});
  return{...evaluated,sourceAdjustedEconomics:evaluated.economics,historyAssessment,comparables:rawComparables,asOf,demoComparables:!hasLiveCompletedSales};
}
