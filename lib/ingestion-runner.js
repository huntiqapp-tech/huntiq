...
const { lookupProduct, prepareRetailerApiIngestion } = require('./retailerapi');
...
async function runProviderJob(job, context) {
  ...
  if (mode === 'dry-run') {
    ...
    if (provider === 'upcitemdb') buildUpcItemDbLookupRequest({ upc: job.upc || job.identifier, useTrial: true });
    if (provider === 'retailerapi' && !(job.identifier || job.upc)) throw new Error('RetailerAPI product identifier is required');
  ...
  if (provider === 'scraper') {
    const batch = await runRetailerScrapeBatch([{
      ...
      fetchImpl,
      asOf: observedAt,
      budget: { maxRecordsPerRun: 1, maxRecordsPerMonth: 1 },
...
async function runIngestion({
  ...
  try {
    const normalized = normalizeJobs(jobs, selected).filter((job) => selected.includes(job.provider));
    const ignored = normalizeJobs(jobs, selected).length - normalized.length;
    ...
    const observations = [];
    const identities = [];
    const rejected = [...overCap];
    const processedJobs = [];
    const seenJobs = new Set();
    const seenObservations = new Set();
    let requestCount = 0;
    let duplicateCount = 0;
    let skippedCount = ignored + overCap.length;
