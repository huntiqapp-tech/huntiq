...
function evaluateOpportunity({opportunity={},comparables=[],channels=[],history={},anomaly={},deal={},asOf}={}){
  ...
  const activeComparableCount=(comparables||[]).filter(c=>['active','listed','asking'].includes(String(c&&c.status||'').toLowerCase())).length;
  const resaleForEconomics={...resale,marketValue:preferredValue,resaleConfidence:resale.resaleConfidence,activeCount:activeComparableCount};
  ...
  const velocity=best?CapitalVelocity.evaluateCapitalVelocity({
    economics:best,
    resale:{
      estimatedDaysToSell:best.holdingDays||opportunity.holdingDays||30,
