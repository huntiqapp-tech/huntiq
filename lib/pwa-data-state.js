...
function classifyOpportunityData(opportunity={}, {asOf=new Date().toISOString(),maxLiveAgeHours=6,maxCachedAgeHours=48,requireAuthorityEnvelope=true}={}){
  ...
  if(validation==='shadow'||requested==='shadow'||requested==='shadow-live'||requested==='validation-only'||requested==='unauthorized'||requested==='withheld'){
    return{kind:'validation',label:'VALIDATION ONLY',customerVisible:false,alertEligible:false,ageHours,reason:requested==='unauthorized'||requested==='withheld'?'customer-unauthorized':requested==='validation-only'?'unvalidated-supplied-row':'provider-shadow-validation'};
  }
  if(requested==='demo'||requested==='demonstration'){
    return{kind:'demo',label:'DEMO DATA',customerVisible:true,alertEligible:false,ageHours,reason:'demonstration-data'};
  }
  if(!requested){
    return{kind:'validation',label:'WITHHELD',customerVisible:false,alertEligible:false,ageHours,reason:'unlabeled-origin'};
  }
